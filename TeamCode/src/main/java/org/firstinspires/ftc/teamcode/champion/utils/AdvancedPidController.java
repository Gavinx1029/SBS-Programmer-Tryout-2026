...
public class AdvancedPidController {
     ...
     *
     * @param current    The monitored value, typically as a sensor input.
     * @param target The target value for the system. This will be stored for future calculations.
    ...
    public double calculate(double current, double target) {
        ...
        if (calculationInterval == 0) {
            ...
            dOutput = -1000 * D * (current - lastInput) / calculationInterval;
        }
...
//        Log.d(pidName, String.format("Output: %.4f; P: %.4f; I: %.4f; D: %.4f;", output, pOutput, Ioutput, dOutput));
     ...
     *
     * @param currentValue The monitored value, typically as a sensor input.
     * @return calculated output value for driving the system
     */
